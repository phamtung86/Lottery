...
//		displayResultSpin("Đặc biệt", listSpecialPrize);
	...
	public static void listDe() {
		...
		for (int i = 0; i < listUserBets.size(); i++) {
			for (int j = 0; j < listLoteryResult.getLast().getListSpecialPrize().size(); j++) {
				if (listUserBets.get(i).getBet().getNumberBet()
						.equals(listLoteryResult.getLast().getListSpecialPrize().get(j).substring(3, 5))
						&& listUserBets.get(i).getBet().getType().equals("Đề")) {
	...
	public static void listLo() {
		...
		for (int i = 0; i < listUserBets.size(); i++) {
			for (int j = 0; j < listAllPrize.size(); j++) {
				if (listUserBets.get(i).getBet().getNumberBet().equals(listAllPrize.get(j))
						&& listUserBets.get(i).getBet().getType().equals("Lô")) {
					System.out.println(listUserBets.get(i));
					System.out.println("Số tiền " + listUserBets.get(i).getAcc().getUser().getFullName() + " trúng: "
							+ listUserBets.get(i).getBet().getPriceBet() * 3.5
									* listUserBets.get(i).getBet().getPointBet());
				}
			}
		}
	}
	...
	public static void displayAccountBet() {
		for (UserBet ub : listUserBets) {
